  ...
  useEffect(() => {
    if (userAdmins) {
      const transformMembershipData = async()  => {
        ...
        const transformChurches = await transformChurchData(userAdmins.churches)
        const transformBusinesses = await transformBusinessData(userAdmins.businesses)
      ...
      <section className="px-4 py-16 md:p-8 lg:p-16 xl:p-24 w-full">
        <div className="pb-8 mb-8 border-b border-gray-300 lg:w-3/4 xl:w-2/3">
          <h1 className="font-bold text-2xl md:text-3xl pb-4">
            Here are the groups and things you are an admin of in <span className="text-indigo-500">{selectedCommunity?.community_name}</span>
          </h1>
          <p>Any groups, schools, churches you set up in Comm-you-nity will be listed here. You need to select the community before accessing this page.</p>
        </div>
        {businessAdmins.length ?
          <>
          <h2 className="text-lg font-semibold mb-8">Your Businesses</h2>
        ...
        {schoolAdmins.length ?
          <>
          <h2 className="text-lg font-semibold mb-8">Schools You Administer</h2>
          <div className={"grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-8 mb-8 pb-8 border-b border-gray-300"}>
