...
import { transformChurchData, transformGroupData, transformSchoolData } from "@/utils/dataTransformers"
  ...
  useEffect(() => {
    if (userMemberships) {
      const transformMembershipData = async()  => {
        const transformGroups = await transformGroupData(userMemberships.userMemberships.groups)
        const transformSchools = await transformSchoolData(userMemberships.userMemberships.schools)
        const transformChurches = await transformChurchData(userMemberships.userMemberships.churches)
      ...
      <section className="px-4 py-16 md:p-8 lg:p-16 xl:p-24 w-full">
        <div className="pb-8 mb-8 border-b border-gray-300 lg:w-3/4 xl:w-2/3">
          <h1 className="font-bold text-2xl md:text-3xl pb-4">
            Here are your community memberships, <span className="text-indigo-500">{user.username}</span>
          </h1>
          <p>Listed below are your community memberships, including groups and churches that you have joined and schools where you are a parent.</p>
        </div>
        {groupMemberships.length ?
          <>
          <h2 className="text-lg font-semibold mb-8">Your Group Memberships</h2>
          <div className={"grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-8 mb-8 pb-8 border-b border-gray-300"}>
        ...
        {schoolMemberships.length ?
          <>
          <h2 className="text-lg font-semibold mb-8">Your School Memberships</h2>
          <div className={"grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-8 mb-8 pb-8 border-b border-gray-300"}>
        ...
        {churchMemberships.length ?
          <>
          <h2 className="text-lg font-semibold mb-8">Your Church Memberships</h2>
          <div className={"grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-8 mb-8 pb-8 border-b border-gray-300"}>
