...
const LeaveCommButton: React.FC<LeaveCommButtonProps> = ({
  community_id,
  setCommunityMember,
}) => {
  const {
    ...
    try {
      if (community_id && user) {
        ...
        if (savedCommunityMembership) {
          const parsedCommunityMembers: CommunitiesLocalStorage[] = JSON.parse(
            savedCommunityMembership
          );
          const updatedCommunities: CommunitiesLocalStorage[] =
            parsedCommunityMembers.filter(
              (c) => String(c.community_id) !== community_id
            );
